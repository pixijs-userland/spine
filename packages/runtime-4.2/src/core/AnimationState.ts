        ...
        for (let i = 0, n = tracks.length; i < n; i++) {
            ...
            if ((i == 0 && alpha == 1) || blend == MixBlend.add) {
                ...
                for (let ii = 0; ii < timelineCount; ii++) {
                    ...
                    if (timeline instanceof AttachmentTimeline) this.applyAttachmentTimeline(timeline, skeleton, applyTime, blend, attachments);
                    else timeline.apply(skeleton, animationLast, applyTime, applyEvents, alpha, blend, MixDirection.mixIn);
                }
            } else {
