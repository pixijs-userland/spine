import { NumberArrayLike, Pool } from '@pixi-spine/base';
...
export class Triangulator {
    ...
    decompose(verticesArray: Array<number>, triangles: Array<number>): Array<Array<number>> {
        ...
        for (let i = 0, n = triangles.length; i < n; i += 3) {
            ...
            const t2 = triangles[i + 1] << 1;
            const t3 = triangles[i + 2] << 1;
            const x1 = vertices[t1];
            const y1 = vertices[t1 + 1];
            const x2 = vertices[t2];
            const y2 = vertices[t2 + 1];
            const x3 = vertices[t3];
            const y3 = vertices[t3 + 1];
