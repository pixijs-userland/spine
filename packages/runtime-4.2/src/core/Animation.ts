...
import { IAnimation, IHasTextureRegion, ITimeline, MathUtils, NumberArrayLike, StringSet, TransformMode, Utils } from '@pixi-spine/base';
...
export class InheritTimeline extends Timeline implements BoneTimeline {
    ...
    constructor(frameCount: number, boneIndex: number) {
        super(frameCount, [`${Property.transformMode}|${boneIndex}`]);
        this.boneIndex = boneIndex;
    }
     ...
     * @param frame Between 0 and <code>frameCount</code>, inclusive.
     * @param time The frame time in seconds. */
    public setFrame(frame: number, time: number, transformMode: TransformMode) {
        frame *= 2 /* ENTRIES*/;
        this.frames[frame] = time;
        this.frames[frame + 1 /* INHERIT*/] = transformMode;
    }
    ...
    public apply(skeleton: Skeleton, lastTime: number, time: number, events: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
        ...
        if (direction == MixDirection.mixOut) {
            if (blend == MixBlend.setup) bone.transformMode = bone.data.transformMode;
        ...
        if (time < frames[0]) {
            if (blend == MixBlend.setup || blend == MixBlend.first) bone.transformMode = bone.data.transformMode;

            return;
        }
        bone.transformMode = this.frames[Timeline.search(frames, time, 2 /* ENTRIES*/) + 1 /* INHERIT*/];
    }
}
...
export class IkConstraintTimeline extends CurveTimeline {
    ...
    apply(skeleton: Skeleton, lastTime: number, time: number, firedEvents: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
        const constraint: IkConstraint = skeleton.ikConstraints[this.constraintIndex];
...
export class TransformConstraintTimeline extends CurveTimeline {
    ...
    apply(skeleton: Skeleton, lastTime: number, time: number, firedEvents: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
        const constraint: TransformConstraint = skeleton.transformConstraints[this.constraintIndex];
...
export class PathConstraintPositionTimeline extends CurveTimeline1 {
    ...
    apply(skeleton: Skeleton, lastTime: number, time: number, firedEvents: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
        const constraint: PathConstraint = skeleton.pathConstraints[this.constraintIndex];
